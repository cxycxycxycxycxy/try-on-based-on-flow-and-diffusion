...
if opt.mode == "paired":
    os.makedirs('./dataset/test/cloth-warp' + opt.name, exist_ok=True)
    os.makedirs('./dataset/test/cloth-warp-mask' + opt.name, exist_ok=True)
elif opt.mode == "unpaired":
    os.makedirs('./dataset/test/unpaired-cloth-warp' + opt.name, exist_ok=True)
    os.makedirs('./dataset/test/unpaired-cloth-warp-mask' + opt.name, exist_ok=True)


# os.makedirs('sample/' + opt.name, exist_ok=True)
# os.makedirs('sample/' + opt.name + "/cloth-warp/", exist_ok=True)
# os.makedirs('sample/' + opt.name + "/cloth-warp-mask/", exist_ok=True)
# os.makedirs('sample/' + opt.name + "/cloth-warp-mask1/", exist_ok=True)
...
if opt.dataset == 'vitonhd':
    warp_model = AFWM_Vitonhd_lrarms(opt, 51).to(device)
elif opt.dataset == 'dresscode':
    warp_model = AFWM_Dressescode_lrarms(opt, 51).to(device)
warp_model.train()
warp_model.cuda()
...
for ii, data in enumerate(tqdm.tqdm(train_loader)):
    with torch.no_grad():
        ...
        left_warped_full_cloth = F.grid_sample(clothes_left.cuda(), left_last_flow.permute(0, 2, 3, 1), mode='bilinear',
                                               padding_mode='zeros')
        ...
        # ziji
        # warped_mask = F.grid_sample(pre_clothes_edge.cuda(), last_flow.permute(0, 2, 3, 1), mode='bilinear',padding_mode='zeros')
        ...
        for bb in range(bz):
            ...
            if opt.dataset == 'vitonhd':
                ...
                warped_edge_fusion = warped_edge_fusion + \
                                     ...
                                     right_arm_mask * 5 + \
                                     neck_mask * 6
                warped_edge_fusion1 = left_warped_cloth_edge[bb] * left_mask + \
                                      torso_warped_cloth_edge[bb] * torso_mask + \
                                      right_warped_cloth_edge[bb] * right_mask
            elif opt.dataset == 'dresscode':
            ...
            # mask = warped_mask.permute(1,2,0).detach().cpu().numpy().astype(np.uint8)

            cv_img = (eee.permute(1, 2, 0).detach().cpu().numpy() + 1) / 2
            rgb = (cv_img * 255).astype(np.uint8)
            bgr = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR)

            e = (warped_edge_fusion1.permute(1, 2, 0).detach().cpu().numpy() + 1) / 2
            ...
            # save_path = 'sample/' + opt.name + "/cloth-warp/" + person_id
            # save_path_mask = "sample/" + opt.name + "/cloth-warp-mask/" + person_id
            # save_path_mask1 = "sample/" + opt.name + "/cloth-warp-mask1/" + person_id
            #
            # # save_path_mask = "sample/"+opt.name+"/cloth-warp-mask_try/"+person_id
            #
            # cv2.imwrite(save_path, bgr)
            # cv2.imwrite(save_path_mask, eee_edge)
            # cv2.imwrite(save_path_mask1, e)
            # cv2.imwrite(save_path_mask_try, mask)

            if opt.mode == "paired":
                save_path = './dataset/test/cloth-warp/' + person_id
                save_path_mask1 = './dataset/test/cloth-warp-mask' + person_id
            elif opt.mode == "unpaired":
                save_path = './dataset/test/unpaired-cloth-warp/' + person_id
                save_path_mask1 = './dataset/test/unpaired-cloth-warp-mask' + person_id
            cv2.imwrite(save_path, bgr)
            cv2.imwrite(save_path_mask1, e)
